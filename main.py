...
def ALL_Window():
        ...
        while True:
            payload = json.dumps(
                {
                    ...
                    "priceFilters": [
                        str(Money(Minimum_Price_txt.get()))
                        + "-"
                        + str(Money(Maximum_Price_txt.get()))
                    ],
                    "discountFilters": [str(iscount_Combobox_Get())],
                    "nextId": nextId,
            ...
            try:
                ...
                if nextId is None:
                    ...
                    for item in i_want:
                        print(
                            f"{item['c2cItemsName']},{item['c2cItemsId']},{item['showPrice']}"
                        )
    ...
    # 价格区
    Maximum_Price_Label = tk.Label(
        text="物品最大价格", foreground="#ffffff", background="#333333"
    )
    ...
    #########
    Minimum_Price_Label = tk.Label(
        text="物品最小价格", foreground="#ffffff", background="#333333"
    )
    ...
    discount_Label = tk.Label(
        text="打折力度", foreground="#ffffff", background="#333333"
    )
    ...
    ###########
    # 排列顺序
    arrangement_Label = tk.Label(
        text="排序规则", foreground="#ffffff", background="#333333"
    )
    ...
    #############
    variety_item_Label = tk.Label(
        text="搜索种类", foreground="#ffffff", background="#333333"
    )
    ...
    # 仅输出当前排序第一位
    one_print_Label = tk.Label(
        text="仅查找排序第一位", foreground="#ffffff", background="#333333"
    )
    ...
    Sleep_Label = tk.Label(
        text="搜索间隔时间(秒)", foreground="#ffffff", background="#333333"
    )
    Sleep_Label.place(x=150, y=310, width=120, height=30, anchor="nw")
    Sleep_Scale = tk.Scale(
        from_=0, to=5, length=200, resolution=0.1, orient="horizontal"
    )
    Sleep_Scale.place(x=160, y=355, width=100, height=50, anchor="nw")
    # 强行终止程序
    Forced_termination_Label = tk.Label(
        text="手动终止程序", foreground="#ffffff", background="#333333"
    )
    Forced_termination_Label.place(x=290, y=310, width=120, height=30, anchor="nw")
    Forced_termination_Scale = tk.Scale(
        from_=0, to=5, length=200, resolution=5, orient="horizontal", showvalue=False
    )
    Forced_termination_Scale.place(x=300, y=375, width=100, height=30, anchor="nw")
    yes_or_no_Label = tk.Label(
        text="否              是", foreground="#ffffff", background="#333333"
    )
...
def file_folder(text):
    ...
    save_path = frozen_dir.app_path() + rf"/Set/item/"
